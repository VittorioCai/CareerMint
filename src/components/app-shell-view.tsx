...
export function AppShellView({
  ...
  locale,
  dictionary,
}: AppShellProps & { locale: AppLocale; dictionary: Dictionary }) {
  const { common, shell } = dictionary;
  const displayEmail = email ?? common.verifiedAccount;
    ...
    <div className="min-h-screen bg-[var(--canvas)] md:grid md:grid-cols-[244px_minmax(0,1fr)]">
      ...
      >
        ...
        <Link href="/applications/new" className="press button-primary mt-6 flex min-h-11 items-center justify-center px-4 text-sm font-semibold">
          ＋ {shell.newApplication}
        </Link>

        <nav className="mt-6 space-y-0.5" aria-label={shell.primaryNavigation}>
          {appNavigation.map((item) => (
            <NavLink
              key={item.href}
              href={item.href}
              label={shell.nav[item.key]}
            />
          ))}
        </nav>

        <div className="mt-auto shrink-0 border-t border-[var(--line)] px-3 pb-3 pt-3.5">
          <p className="text-xs font-semibold">{shell.safetyTitle}</p>
          <p className="mt-1 text-xs font-normal leading-[1.55] text-[var(--ink-muted)]">{shell.safetyBody}</p>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="ml-auto flex items-center gap-2">
              ...
              <Link
                ...
                aria-label={shell.newApplication}
              ...
              <details className="group relative">
                ...
                <div className="motion-enter absolute right-0 top-[calc(100%+8px)] z-30 w-64 rounded-2xl border border-[var(--line)] bg-[var(--paper)] p-2 shadow-[var(--elevation-2)]">
                  <div className="border-b border-[var(--line)] px-3 py-2.5">
                    <p className="type-eyebrow text-[var(--ink-muted)]">{common.verifiedAccount}</p>
                    <p className="mt-1 truncate text-sm font-bold">{displayEmail}</p>
                  </div>
                  <Link href="/settings/account" className="mt-1 block rounded-lg px-3 py-2 text-sm font-bold hover:bg-[var(--canvas)]">{shell.accountSettings}</Link>
                  <Link href="/settings/privacy" className="block rounded-lg px-3 py-2 text-sm font-bold hover:bg-[var(--canvas)]">{shell.privacySettings}</Link>
                  <div className="mt-1 flex items-center justify-between gap-3 border-t border-[var(--line)] px-3 pt-2">
                    ...
                    <LanguageSwitch
                      ...
                      onFailure={shell.localeNotSaved}
                    />
                  </div>
                  <form action={signOut} className="mt-1 border-t border-[var(--line)] pt-1">
                    <button type="submit" className="w-full rounded-lg px-3 py-2 text-left text-sm font-bold text-[var(--error)] hover:bg-[var(--danger-tint)]">{shell.signOut}</button>
                  </form>
                </div>
              </details>
            </div>
