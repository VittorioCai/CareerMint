  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-[var(--coral)] bg-[var(--paper)] p-5 sm:p-7">
        ...
        <button type="button" className="mt-5 min-h-11 rounded-xl border-2 border-[var(--error)] px-5 text-sm font-semibold text-[var(--error)]" onClick={() => setDialogOpen(true)}>
          删除我的账户
        </button>
      </section>
      ...
      {dialogOpen ? (
        <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-[color:var(--ink)]/40 p-4">
          <section role="dialog" aria-modal="true" aria-label="确认删除账户" className="soft-surface w-full max-w-lg p-5 sm:p-7">
            ...
            <div className="mt-6 flex flex-wrap justify-end gap-2">
              <button type="button" className="button-secondary min-h-10 px-4 text-sm font-semibold" disabled={busy} onClick={() => setDialogOpen(false)}>取消</button>
