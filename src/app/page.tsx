    ...
    <main className="landing-shell min-h-screen overflow-hidden">
      <nav className="mx-auto flex w-full max-w-[1180px] items-center justify-between px-5 py-5 sm:px-8 lg:px-10" aria-label="公开导航">
        <Link href="/" className="group flex items-center gap-3" aria-label="求职搭子首页">
          ...
          <span className="hidden rounded-full border border-[color:var(--ink-soft)] bg-[var(--paper)] px-2.5 py-1 type-eyebrow sm:inline">Beta</span>
        </Link>
      ...
      <section className="mx-auto grid w-full max-w-[1180px] gap-12 px-5 pb-20 pt-10 sm:px-8 lg:grid-cols-[0.94fr_1.06fr] lg:items-center lg:px-10 lg:pb-28 lg:pt-16">
        <div className="relative z-10 max-w-[620px]">
          ...
          <h1 className="type-display heading-font">
            让每次申请
            <span className="mt-2 block">都有依据</span>
          </h1>
          ...
          <ul className="mt-7 flex flex-wrap gap-x-6 gap-y-2 text-sm font-semibold text-[var(--ink-muted)]" aria-label="产品原则">
            <li className="flex items-center gap-2"><span className="size-2 rounded-full bg-[var(--sev-matched-ink)]" />事实有来源</li>
            <li className="flex items-center gap-2"><span className="size-2 rounded-full bg-[var(--sev-minor-ink)]" />修改可解释</li>
            <li className="flex items-center gap-2"><span className="size-2 rounded-full bg-[var(--sev-critical-ink)]" />写入先确认</li>
          </ul>
        </div>
        ...
        <div className="relative mx-auto w-full max-w-[620px] lg:mx-0">
          <div className="soft-surface relative overflow-hidden">
            <div className="flex items-center justify-between border-b border-[var(--line)] px-5 py-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.13em] text-[var(--ink-muted)]">申请工作区</p>
                <p className="heading-font mt-1 text-xl font-semibold">Senior Product Manager</p>
              </div>
              <div className="rounded-full bg-[var(--surface-muted)] px-3 py-1.5 text-xs font-semibold">1 项待确认</div>
            </div>

            <div className="grid sm:grid-cols-[1fr_180px]">
              <div className="p-5 sm:p-6">
                <div className="mb-5 flex items-center justify-between">
                  <p className="text-sm font-semibold">岗位要求</p>
                ...
                <div className="mt-6 border-t border-dashed border-[var(--ink-soft)] pt-5">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold">有一项值得补充</p>
              ...
              <aside className="border-t border-[var(--line)] bg-[var(--canvas)] p-5 sm:border-l sm:border-t-0" aria-label="申请状态">
                <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[var(--ink-muted)]">下一步</p>
                <p className="heading-font mt-2 text-lg font-semibold">确认 1 条事实</p>
                <div
                ...
                <button type="button" className="button-secondary mt-6 w-full px-3 py-2.5 text-sm font-semibold">查看建议</button>
              </aside>
            </div>
          </div>

          <div className="absolute -right-3 -top-5 rounded-lg border border-[var(--line)] bg-[var(--paper)] px-3 py-1.5 text-xs font-semibold shadow-[var(--elevation-1)] sm:right-8">AI 建议有证据 ↗</div>
        </div>
      </section>
