...
当前可用流程：注册与登录 → 设置求职目标 → 上传 PDF/DOCX → 用户授权后进行 AI 文字分析 → 逐条确认事实 → 添加 JD 并建立申请工作区 → 用户点击后解析 JD 并匹配已确认事实 → 生成岗位简历建议 → 逐条接受、编辑或拒绝 → 保存不可变 V1/V2 → 在看板或表格中管理投递 → 更新阶段并保留时间线 → 导出全部个人数据或删除账户。文档导出与面试准备属于后续开发计划。
...
本仓库已经完成账户、职业事实、申请工作区、JD 解析与已确认事实匹配、岗位简历建议的逐条审核、不可变简历版本、投递看板/表格、阶段更新时间线和首页进度摘要。以下功能会在后续计划实现：DOCX/PDF 导出、深度统计、岗位专属面试问题和回答提纲。自动抓取招聘网站、自动投递、LinkedIn 自动化和虚假经历生成不在 MVP 范围内。
