...
当前可用流程：注册与登录 → 设置求职目标 → 上传 PDF/DOCX → 用户授权后进行 AI 文字分析 → 逐条确认事实 → 添加 JD 并建立申请工作区 → 用户点击后解析 JD 并匹配已确认事实 → 在看板或表格中管理投递 → 更新阶段并保留时间线 → 导出全部个人数据或删除账户。简历定制和面试准备已经保留工作区入口，属于后续开发计划。
...
没有真实 `DEEPSEEK_API_KEY` 时，简历提取和 JD 分析会显示可恢复的“AI 暂不可用”状态，不会产生模型费用。单元测试和本地 E2E 使用 mock/fake provider，不会请求 DeepSeek。
...
`E2E_FAKE_EXTRACTOR=1` 仅在非生产环境启用确定性的简历提取与 JD 分析。E2E 会创建并最终删除随机测试账户。
...
本仓库已经完成账户、职业事实、申请工作区、JD 解析与已确认事实匹配、投递看板/表格、阶段更新时间线和首页进度摘要。以下功能会在后续计划实现：岗位定制简历、深度统计、岗位专属面试问题和回答提纲。自动抓取招聘网站、自动投递、LinkedIn 自动化和虚假经历生成不在 MVP 范围内。
